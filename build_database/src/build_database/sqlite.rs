...
use crate::{build_log_info, error::CliError, migrations::run_sqlite_migrations};

use super::SplinterEnvironment;

const DEFAULT_SQLITE: &str = "ai_hero_state.db";
...
pub fn sqlite_migrations(connection_string: String) -> Result<(), CliError> {
    ...
    if connection_string != MEMORY {
        ...
        build_log_info!("Running migrations against SQLite database: :memory: ");
    };

    run_sqlite_migrations(&mut *pool.get().map_err(|_| {
